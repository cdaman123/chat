...
1. Visit the [Releases page](https://github.com/cdaman123/chat/releases/), choose the latest or otherwise the most suitable release. From the list of binaries download the one for your database and platform. Once the binary is downloaded, unpack it to a directory of your choosing, `cd` to that directory.
...
4. Fetch, build Tinode server and tinode-db database initializer:
  - **MySQL**:
	```
	go install -tags mysql github.com/cdaman123/chat/server@latest
	go install -tags mysql github.com/cdaman123/chat/tinode-db@latest
	```
  - **MongoDB**:
	```
	go install -tags mongodb github.com/cdaman123/chat/server@latest
	go install -tags mongodb github.com/cdaman123/chat/tinode-db@latest
	```
  - **RethinkDb**:
	```
	go install -tags rethinkdb github.com/cdaman123/chat/server@latest
	go install -tags rethinkdb github.com/cdaman123/chat/tinode-db@latest
	```
  - **All** (bundle all of the above DB adapters):
	```
	go install -tags "mysql rethinkdb mongodb" github.com/cdaman123/chat/server@latest
	go install -tags "mysql rethinkdb mongodb" github.com/cdaman123/chat/tinode-db@latest
	```

    The steps above install Tinode binaries at `$GOPATH/bin/`, sorces and supporting files are located at `$GOPATH/pkg/mod/github.com/cdaman123/chat@vX.XX.X/` where `X.XX.X` is the version you installed, such as `0.19.1`.
    ...
    ```
    go install -tags mysql -ldflags "-X main.buildstamp=`date -u '+%Y%m%dT%H:%M:%SZ'`" github.com/cdaman123/chat/server@latest
    ```
    ...
    ```
    go get -tags mysql github.com/cdaman123/chat/server && go build -tags mysql -o $GOPATH/bin/tinode github.com/cdaman123/chat/server
    ```
    Building with Go 1.13 or below **will fail**!

5. Open `tinode.conf` (located at `$GOPATH/pkg/mod/github.com/cdaman123/chat@vX.XX.X/server/`). Check that the database connection parameters are correct for your database. If you are using MySQL make sure [DSN](https://github.com/go-sql-driver/mysql#dsn-data-source-name) in `"mysql"` section is appropriate for your MySQL installation. Option `parseTime=true` is required.
```js
	"mysql": {
...
If you followed instructions in the previous section then the Tinode binaries are installed in `$GOPATH/bin/`, the sources and supporting files are located in `$GOPATH/pkg/mod/github.com/cdaman123/chat@vX.XX.X/`, where `X.XX.X` is the version you installed, for example `0.19.1`.

Switch to sources directory (replace `X.XX.X` with your actual version, such as `0.19.1`):
```
cd $GOPATH/pkg/mod/github.com/cdaman123/chat@vX.XX.X
```
...
3. Unpack JS client to a directory, for instance `$HOME/cdaman123/webapp/` by unzipping `https://github.com/cdaman123/webapp/archive/master.zip` and `https://github.com/cdaman123/tinode-js/archive/master.zip` to the same directory.
...
5. Run the server
	```
	$GOPATH/bin/tinode -config=./server/tinode.conf -static_data=$HOME/cdaman123/webapp/
	```
...
```
nohup $GOPATH/bin/server -config=./server/tinode.conf -static_data=$HOME/cdaman123/webapp/ &
exit
```
...
For more details see https://github.com/cdaman123/chat/issues/25.
